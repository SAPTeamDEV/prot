...
class TeamsScoreScreenActivity(bsGame.ScoreScreenActivity):
                ...
                subs=[
                    (
                        ...
                        bs.Lstr(
                            resource="upNextText",
                            subs=[
                                ("${COUNT}", str(self.getSession().getGameNumber() + 1))
                            ],
                        ),
                    ),
    ...
    def showPlayerScores(
        ...
        if results is not None:
            ...
            playersSorted = [
                [_getPlayerScore(p), name, p]
                for name, p in self.scoreSet.getValidPlayers().items()
            ]
            playersSorted.sort(
                reverse=(results is None or not results._getLowerIsBetter())
            )
        ...
        def _txt(xOffs, yOffs, text, hAlign="right", extraScale=1.0, maxWidth=120.0):
            bsUtils.Text(
                ...
                position=(
                    tsHOffs + xOffs * scale,
                    tsVOffset + (vOffs + yOffs + 4.0) * scale,
                ),
                hAlign=hAlign,
                vAlign="center",
        ...
        def _scoreTxt(text, xOffs, highlight, delay, maxWidth=70.0):
            bsUtils.Text(
                text,
                position=(tsHOffs + xOffs * scale, tsVOffset + (vOffs + 15) * scale),
                scale=scale,
        ...
        for p in playersSorted:
            ...
            bsUtils.Image(
                p.getIcon(),
                position=(tsHOffs - 12 * scale, tsVOffset + (vOffs + 15.0) * scale),
            ...
            bsUtils.Text(
                ...
                maxWidth=160,
                scale=0.75 * scale,
                position=(tsHOffs + 10.0 * scale, tsVOffset + (vOffs + 15) * scale),
                hAlign="left",
                vAlign="center",
            ...
            _scoreTxt(
                str(p.accumKilledCount), 280, p.accumKilledCount == topKilledCount, 100
            )
...
class FreeForAllVictoryScoreScreenActivity(TeamsScoreScreenActivity):
    ...
    def onBegin(self):
        ...
        self.showPlayerScores(
            delay=1, results=self.settings["results"], scale=1.2, xOffset=-110
        )
        ...
        for i, player in enumerate(playerOrderPrev):
            ...
            bs.gameTimer(
                tDelay + 300, bs.Call(bs.playSound, self._scoreDisplaySoundSmall)
            )
            if orderChange:
                bs.gameTimer(
                    tDelay + delay2 + 100, bs.Call(bs.playSound, self._cymbalSound)
                )

            img = bsUtils.Image(
                player.getIcon(),
                position=(tsHOffs - 72.0 * scale, yBase + (vOffs + 15.0) * scale),
                scale=(30.0 * scale, 30.0 * scale),
                transition="inLeft",
                transitionDelay=tDelay,
            ).autoRetain()
            bs.gameTimer(
                ...
                bs.WeakCall(
                    ...
                    "input1",
                    {
                        0: yBase + (vOffs + 15.0) * scale,
                        transitionTime: yBase + (vOffs2 + 15.0) * scale,
                    },
                ),
            )
            bs.gameTimer(
                ...
                bs.WeakCall(
                    ...
                    "input0",
                    {
                        0: tsHOffs - 72.0 * scale,
                        transitionTime2: tsHOffs - (72.0 + slideAmt) * scale,
                    },
                ),
            )
            txt = bsUtils.Text(
                ...
                maxWidth=130.0,
                scale=0.75 * scale,
                position=(tsHOffs - 50.0 * scale, yBase + (vOffs + 15.0) * scale),
                hAlign="left",
                vAlign="center",
                color=bs.getSafeColor(player.getTeam().color + (1,)),
                transition="inLeft",
                transitionDelay=tDelay,
            ).autoRetain()
            bs.gameTimer(
            ...
            bs.gameTimer(
                ...
                bs.WeakCall(
                    ...
                    "input0",
                    {
                        0: tsHOffs - 50.0 * scale,
                        transitionTime2: tsHOffs - (50.0 + slideAmt) * scale,
                    },
                ),
            )
            ...
            txtNum = bsUtils.Text(
                "#" + str(i + 1),
                scale=0.55 * scale,
                position=(tsHOffs - 95.0 * scale, yBase + (vOffs + 8.0) * scale),
                hAlign="right",
            ...
            bs.gameTimer(
                ...
                bs.WeakCall(
                    self._safeAnimate,
                    txtNum.positionCombine,
                    "input0",
                    {
                        0: tsHOffs - 95.0 * scale,
                        transitionTime2: tsHOffs - (95.0 + slideAmt) * scale,
                    },
                ),
            )

            sTxt = _scoreTxt(
                str(player.getTeam().sessionData["previousScore"]), 80, 0, False, 0, 1.0
            )
            bs.gameTimer(
                tDelay + delay2,
                bs.WeakCall(
                    self._safeAnimate,
                    sTxt.positionCombine,
                    "input1",
                    {
                        0: yBase + (vOffs + 2.0) * scale,
                        transitionTime: yBase + (vOffs2 + 2.0) * scale,
                    },
                ),
            )
            bs.gameTimer(
            ...
            if scoreChange > 0:
                ...
                y = 3.0
                sTxt2 = _scoreTxt(
                    "+" + str(scoreChange), x, y, True, 0, 0.7, flash=True
                )
                bs.gameTimer(
                    tDelay + delay2,
                    bs.WeakCall(
                        self._safeAnimate,
                        sTxt2.positionCombine,
                        "input1",
                        {
                            0: yBase + (vOffs + y + 2.0) * scale,
                            transitionTime: yBase + (vOffs2 + y + 2.0) * scale,
                        },
                    ),
                )
                bs.gameTimer(
                    ...
                    bs.WeakCall(
                        ...
                        {
                            0: tsHOffs + x * scale,
                            transitionTime2: tsHOffs + (x - slideAmt) * scale,
                        },
                    ),
                )
                ...
                bs.gameTimer(
                    tDelay + delay1,
                    bs.Call(_safeSetAttr, sTxt.node, "color", (1, 1, 1, 1)),
                )
                for j in range(scoreChange):
                    ...
                    if not t in soundTimes:
                        soundTimes.add(t)
                        bs.gameTimer(
                            t, bs.Call(bs.playSound, self._scoreDisplaySoundSmall)
                        )
...
class TeamVictoryScoreScreenActivity(TeamsScoreScreenActivity):
    ...
    def onBegin(self):
        ...
        if bsUtils._getResource("bestOfUseFirstToInstead"):
            bestTxt = bs.Lstr(
                resource="firstToSeriesText",
                subs=[("${COUNT}", str(self.getSession()._seriesLength / 2 + 1))],
            )
        else:
            bestTxt = bs.Lstr(
                resource="bestOfSeriesText",
                subs=[("${COUNT}", str(self.getSession()._seriesLength))],
            )
        ...
        for team in self.teams:
            ...
            bs.gameTimer(
                i * 150 + 500, bs.Call(bs.playSound, self._scoreDisplaySoundSmall)
            )
            ...
            if scored:
                ...
                bs.gameTimer(
                    i * 150 + 1500, bs.Call(bs.playSound, self._scoreDisplaySound)
                )
...
class TeamSeriesVictoryScoreScreenActivity(TeamsScoreScreenActivity):
    ...
    def onBegin(self):
        ...
        if self._isFFA:
            playersSorted = [
                [p.getTeam().sessionData["score"], p.getName(full=True), p]
                for name, p in self.scoreSet.getValidPlayers().items()
                ...
                subs=[
                    (
                        ...
                        bs.Lstr(
                            ...
                            subs=[
                                ("${COUNT}", str(self.getSession()._ffaSeriesLength))
                            ],
                        ),
                    )
                ],
        ...
        for score, name, p in playersSorted:
            ...
            vOffs -= 40
            bsUtils.Text(
                str(p.getTeam().sessionData["score"]) if self._isFFA else str(p.score),
                color=(0.5, 0.5, 0.5, 1.0),
    ...
    def _showWinner(self, team):
        if not self._isFFA:
            ...
            if len(team.players) == 1:
                i = bsUtils.Image(
                    team.players[0].getIcon(), position=(0, 143), scale=(100, 100)
                ).autoRetain()
                bsUtils.animate(i.node, "opacity", {0: 0.0, 250: 1.0})
                bsUtils.ZoomText(
                    bs.Lstr(value=team.players[0].getName(full=True, icon=False)),
                    position=(0, 97 + offsV),
                    color=team.color,
                    scale=1.15,
                    jitter=1.0,
...
class TeamGameActivity(bs.GameActivity):
    ...
    def end(
        self, results=None, announceWinningTeam=True, announceDelay=100, force=False
    ):
        """
...
class TeamBaseSession(bs.Session):
    ...
    def __init__(self):
        ...
        if showTutorial:
            # get this loading..
            self._tutorialActivityInstance = bs.newActivity(bsTutorial.TutorialActivity)
        else:
    ...
    def _instantiateNextGame(self):
        self._nextGameInstance = bs.newActivity(
            self._nextGameSpec["resolvedType"], self._nextGameSpec["settings"]
        )
    ...
    def onActivityEnd(self, activity, results):
        ...
        # hop into a round
            ...
            isinstance(activity, bsGame.JoiningActivity)
        ...
        # if we're leaving a round, go to the score screen
            ...
            if self._useTeams:
                ...
                if len(winners) < 2:
                    ...
                    if winner.sessionData["score"] >= (self._seriesLength - 1) / 2 + 1:
                        self.setActivity(
                            bs.newActivity(
            ...
            # free-for-all mode
                ...
                if len(self.players) > 1 and len(winners) < 2:
                    self.setActivity(
                        bs.newActivity(DrawScoreScreenActivity, {"results": results})
                    )
                else:
                    ...
                    for i, winner in enumerate(winners):
                        for team in winner["teams"]:
                            ...
                            team.sessionData["previousScore"] = team.sessionData[
                                "score"
                            ]
                    ...
                    seriesWinners = [
                        team
                        for team in self.teams
                        if team.sessionData["score"] >= self._ffaSeriesLength
                    ]
                    seriesWinners.sort(
                        reverse=True, key=lambda team: (team.sessionData["score"])
                    )
    ...
    def announceGameResults(self, activity, results, delay, announceWinningTeam=True):
        ...
        if announceWinningTeam:
            ...
            if winningTeam is not None:
                ...
                winsText = bs.Lstr(
                    resource=winsResource, subs=[("${NAME}", winningTeam.name)]
                )
                activity.showZoomMessage(
                    winsText,
                    scale=0.85,
                    color=bsUtils.getNormalizedColor(winningTeam.color),
                )
