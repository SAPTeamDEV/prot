...
class ConquestGame(bs.TeamGameActivity):
    ...
    def onBegin(self):
        ...
        for i in range(len(self.getMap().flagPoints)):
            ...
            flag = ConquestFlag(
                position=point, touchable=False, materials=[self._extraFlagMaterial]
            )
    ...
    def _updateScores(self):
        ...
        for team in self.teams:
            ...
            self._scoreBoard.setTeamValue(
                team, team.gameData["flagsHeld"], len(self._flags)
            )
    ...
    def _handleFlagPlayerCollide(self):
        ...
        if flag.getTeam() is not player.getTeam():
            ...
            for p in self.players:
                if (
                    p.getTeam() is flag.getTeam()
                    and p.actor is not None
                    and not p.isAlive()
    ...
    def _getPlayerSpawnPosition(self, player):
        ...
        spawns = [
            i for i in range(spawnCount) if self._flags[i].getTeam() is player.getTeam()
        ]
