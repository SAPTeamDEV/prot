...
class HockeyGame(bs.TeamGameActivity):
    ...
    def getSettings(cls, sessionType):
        return [
            ("Score to Win", {"minValue": 1, "default": 1, "increment": 1}),
            (
                "Time Limit",
                {
                    "choices": [
    ...
    def _handlescore(self):
        ...
        for team in self.teams:
            if team.getID() == i:
                ...
                if (
                    scoringteam.getID() in self._puck.lastplayerstotouch
                    and self._puck.lastplayerstotouch[scoringteam.getID()].exists()
                ):
