...
class KingOfTheHillGame(bs.TeamGameActivity):
    ...
    def getSettings(cls, sessionType):
        return [
            ("Hold Time", {"minValue": 10, "default": 30, "increment": 10}),
            (
                "Time Limit",
                {
                    "choices": [
    ...
    def onBegin(self):
        ...
        self._flag = bs.Flag(position=self._flagPos, touchable=False, color=(1, 1, 1))
        self._flagLight = bs.newNode(
            "light",
            attrs={
    ...
    def _tick(self):
        ...
        if scoringTeam:
            ...
            try:
                bs.playSound(
                    self._countDownSounds[scoringTeam.gameData["timeRemaining"]]
                )
            except Exception:
                pass
    ...
    def endGame(self):
        ...
        for team in self.teams:
            results.setTeamScore(
                team, self.settings["Hold Time"] - team.gameData["timeRemaining"]
            )
        self.end(results=results, announceDelay=0)

    def _updateFlagState(self):
        holdingTeams = set(
            player.getTeam() for player in self.players if player.gameData["atFlag"]
        )
