...
class BombFactory(object):
    ...
    def __init__(self):
        ...
        self.bombMaterial.addActions(
            conditions=(
                (("weAreYoungerThan", 100), "or", ("theyAreYoungerThan", 100)),
                "and",
        ...
        self.landMineBlastMaterial.addActions(
            conditions=(
                ("weAreOlderThan", 200),
                "and",
                ("theyAreOlderThan", 200),
                "and",
                ("evalColliding",),
                "and",
                (
                    ("theyDontHaveMaterial", self.landMineNoExplodeMaterial),
                    "and",
                    (
                        ("theyHaveMaterial", bs.getSharedObject("objectMaterial")),
                        "or",
                        ("theyHaveMaterial", bs.getSharedObject("playerMaterial")),
                    ),
                ),
            ),
        ...
        self.dinkSounds = (bs.getSound("bombDrop01"), bs.getSound("bombDrop02"))
        ...
        self.normalSoundMaterial.addActions(
            ...
            actions=(
                ("impactSound", self.dinkSounds, 2, 0.8),
    ...
    def __init__(
        ...
        light = bs.newNode(
            ...
            attrs={
                ...
                "color": (
                    (0.6, 0.6, 1.0) if self.blastType == "ice" else (1, 0.3, 0.1)
                ),
            },
        )
...
class Bomb(bs.Actor):
    ...
    def __init__(
        ...
        if self.bombType == "landMine":
            ...
            self.warnTimer = bs.Timer(
                fuseTime - 1700, bs.WeakCall(self.handleMessage, WarnMessage())
            )
            ...
            sound = bs.newNode(
                "sound",
                owner=self.node,
                attrs={"sound": factory.fuseSound, "volume": 0.25},
            )
    ...
    def _handleSplat(self, m):
        ...
        if (
            ...
            bs.playSound(
                self.getFactory().stickyImpactSound, 2.0, position=self.node.position
            )
    ...
    def arm(self):
        ...
        factory = self.getFactory()
        if self.bombType == "landMine":
            self.textureSequence = bs.newNode(
                "textureSequence",
                owner=self.node,
                attrs={
                    "rate": 30,
            ...
            bs.gameTimer(
                250, bs.WeakCall(self._addMaterial, factory.landMineBlastMaterial)
            )
        elif self.bombType == "impact":
            self.textureSequence = bs.newNode(
                "textureSequence",
