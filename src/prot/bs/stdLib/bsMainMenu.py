...
class MainMenuActivity(bs.Activity):
    ...
    def onTransitionIn(self):
            ...
            self.myName = bs.NodeActor(
                bs.newNode(
                    ...
                    attrs={
                        ...
                        if vrMode
                        else (0.5, 0.6, 0.5, 0.6),
                        "flatness": 1.0,
                        "shadow": 1.0 if vrMode else 0.5,
                        "scale": (
                            0.9 if (env["interfaceType"] == "small" or vrMode) else 0.7
                        ),  # FIXME need a node attr for this
                        "position": (0, 10),
                        "vrDepth": -10,
        ...
        if False:
            node = bs.newNode(
                "timeDisplay",
                attrs={"timeMin": 2000, "timeMax": 10000, "showSubSeconds": True},
            )
            self._fooText = bs.NodeActor(
                bs.newNode(
                    "text",
                    attrs={"position": (0, -220), "flatness": 1.0, "hAlign": "center"},
                )
            )
        ...
        class News(object):
            def __init__(self, activity):
                ...
                self._fetchTimer = bs.Timer(
                    1000, bs.WeakCall(self._tryFetchingNews), repeat=True
                )
        ...
        with bs.Context("UI"):
            ...
            # ..or in normal cases go back to the main menu
                ...
                if mainWindow == "Gather":
                    bsUI.uiGlobals["mainMenuWindow"] = bsUI.GatherWindow(
                        transition=None
                    ).getRootWidget()
                elif mainWindow == "Watch":
                    bsUI.uiGlobals["mainMenuWindow"] = bsUI.WatchWindow(
                        transition=None
                    ).getRootWidget()
                elif mainWindow == "Team Game Select":
                    bsUI.uiGlobals["mainMenuWindow"] = bsUI.TeamsWindow(
                        sessionType=bs.TeamsSession, transition=None
                    ).getRootWidget()
                elif mainWindow == "Free-for-All Game Select":
                    bsUI.uiGlobals["mainMenuWindow"] = bsUI.TeamsWindow(
                        sessionType=bs.FreeForAllSession, transition=None
                    ).getRootWidget()
                elif mainWindow == "Coop Select":
                    bsUI.uiGlobals["mainMenuWindow"] = bsUI.CoopWindow(
                        transition=None
                    ).getRootWidget()
                else:
                    bsUI.uiGlobals["mainMenuWindow"] = bsUI.MainMenuWindow(
                        transition=None
                    ).getRootWidget()
        ...
        if l != self._language:
            ...
            if bs.getLanguage() == "Chinese":
                ...
                for shadow in (True, False):
                    ...
                    delay += delayInc
                    self._makeWord(
                        "m", x, y + yExtra, delay=delay, scale=gScale, shadow=shadow
                    )
                    x += spacing * 1.25
                    delay += delayInc
                    self._makeWord(
                    ...
                    delay += delayInc
                    self._makeWord(
                        "q", x, y + yExtra, delay=delay, scale=gScale, shadow=shadow
                    )
                    x += spacing * 0.9
                    delay += delayInc
                    self._makeWord(
                    ...
                    delay += delayInc
                    self._makeWord(
                        "a", x, y + yExtra, delay=delay, scale=gScale, shadow=shadow
                    )
                    x += spacing * 0.64
                    delay += delayInc
                    self._makeWord(
                ...
                self._makeLogo(
                    baseX - 28, 125 + y + 1.2 * yExtra, 0.32 * gScale, delay=baseDelay
                )
    ...
    def _makeWord(self, word, x, y, scale=1.0, delay=0, vrDepthOffset=0, shadow=False):
        ...
        if not bs.getEnvironment()["vrMode"]:
            if not shadow:
                ...
                c = None
            if shadow:
                c2 = bs.newNode("combine", owner=wordShadowObj.node, attrs={"size": 2})
            else:
                c2 = None
            if not shadow:
...
class SplashScreenActivity(bs.Activity):
    ...
    def _startPart2(self):
        ...
        self._part = 2
        self._image = bsUtils.Image(
            self._tex2, transition="fadeIn", scale=(400, 400), transitionDelay=0
        )
        t = bsUtils._translate(
            "tips",
