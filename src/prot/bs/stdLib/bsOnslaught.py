...
class OnslaughtGame(bs.CoopGameActivity):
    ...
    def onBegin(self):
        ...
        if self._preset in ["training", "trainingEasy"]:
            ...
            bsUtils.ControlsHelpOverlay(
                delay=3000, lifespan=10000, bright=True
            ).autoRetain()
            ...
            self._waves = [
                {
                    "entries": [
                        {"type": bs.NinjaBot, "point": "LeftUpperMore"}
                        if playerCount > 2
                        else None,
                ...
                },
                {
                    "entries": [
                        ...
                        {"type": bs.BomberBotStaticLame, "point": "TurretBottomRight"}
                        if playerCount > 2
                        else None,
                    ]
                },
                {
                    "entries": [
                        ...
                        {"type": bs.ChickBot, "point": "LeftUpper"}
                        if playerCount > 2
                        else None,
                    ]
                },
                {
                    "entries": [
                        ...
                        {"type": bs.ToughGuyBot, "point": "TopHalfRight"}
                        if playerCount > 1
                        else None,
                        {"type": bs.ToughGuyBotLame, "point": "TopLeft"},
                        {"type": bs.ToughGuyBotLame, "point": "TopHalfLeft"}
                        if playerCount > 2
                        else None,
                ...
                },
                {
                    "entries": [
                        ...
                        {"type": bs.NinjaBot, "point": "BottomHalfLeft"}
                        if playerCount > 1
                        else None,
            ...
            self._waves = [
                ...
                },
                {
                    "entries": [
                        ...
                        {"type": bs.NinjaBot, "point": "RightUpperMore"}
                        if playerCount > 2
                        else None,
                        {"type": bs.BomberBotProStatic, "point": "TurretTopLeft"},
                ...
                },
                {
                    "entries": [
                        ...
                        {"type": bs.ChickBotPro, "point": "RightUpperMore"}
                        if playerCount > 1
                        else None,
                        {"type": bs.ChickBotPro, "point": "RightUpper"},
                        {"type": bs.ChickBotPro, "point": "RightLower"}
                        if hard
                        else None,
                        {"type": bs.ChickBotPro, "point": "RightLowerMore"}
                        if playerCount > 2
                        else None,
                ...
                },
                {
                    "entries": [
                        ...
                        {"type": bs.NinjaBotProShielded, "point": "Bottom"}
                        if playerCount > 2
                        else None,
                        {"type": bs.NinjaBotProShielded, "point": "BottomLeft"},
                        {"type": bs.NinjaBotProShielded, "point": "Top"}
                        if hard
                        else None,
        ...
        self._dropPowerups(
            ...
            powerupType="curse"
            if self._preset in ["uber", "uberEasy"]
            else ("landMines" if self._preset in ["rookie", "rookieEasy"] else None),
        )
    ...
    def _dropPowerup(self, index, powerupType=None):
        ...
        bs.Powerup(
            position=self.getMap().powerupSpawnPoints[index], powerupType=powerupType
        ).autoRetain()
    ...
    def _dropPowerups(self, standardPoints=False, powerupType=None):
        ...
        if standardPoints:
            ...
            for i, pt in enumerate(pts):
                bs.gameTimer(
                    1000 + i * 500,
                    bs.WeakCall(self._dropPowerup, i, powerupType if i == 0 else None),
                )
        else:
            pt = (
                self._powerupCenter[0]
                + random.uniform(
                    -1.0 * self._powerupSpread[0], 1.0 * self._powerupSpread[0]
                ),
                self._powerupCenter[1],
                self._powerupCenter[2]
                + random.uniform(-self._powerupSpread[1], self._powerupSpread[1]),
            )
    ...
    def _updateWaves(self):
        ...
        if self._canEndWave and not self._bots.haveLivingBots() and not self._gameOver:
            ...
            if self._timeBonus > 0:
                ...
                bs.gameTimer(
                    baseDelay, bs.WeakCall(self._awardTimeBonus, self._timeBonus)
                )
                baseDelay += 1000
            ...
            if self._wave > 0:
                ...
                for player in self.players:
                    if player.isAlive() and player.gameData["hasBeenHurt"] == False:
                        haveFlawless = True
                        bs.gameTimer(
                            baseDelay, bs.WeakCall(self._awardFlawlessBonus, player)
                        )
            ...
            if won:
                self.showZoomMessage(
                    bs.Lstr(resource="victoryText"), scale=1.0, duration=4000
                )
            ...
            try:
                if player.isAlive():
                    self.scoreSet.playerScored(
                        player,
                        int(100 / len(self.initialPlayerInfo)),
                        scale=1.4,
    ...
    def _startTimeBonusTimer(self):
        self._timeBonusTimer = bs.Timer(
            1000, bs.WeakCall(self._updateTimeBonus), repeat=True
        )
        ...
        if not any(player.isAlive() for player in self.teams[0].players):
            ...
            for player in self.players:
                if not player.isAlive() and (
                    ...
                    or (player.gameData["respawnWave"] <= len(self._waves))
                ):
                    t = bs.Lstr(
                        value="${A}${B}\n",
                        subs=[
                            ...
                            (
                                ...
                                bs.Lstr(
                                    ...
                                    subs=[
                                        ...
                                        (
                                            "${WAVE}",
                                            str(player.gameData["respawnWave"]),
                                        ),
                                    ],
                                ),
                            ),
    ...
    def _startNextWave(self):
        ...
        if self._preset in ["endless", "endlessTournament"]:
            ...
            botLevels = [
                [b for b in botTypes2 if b.pointsMult == 1],
            ...
            for group in distribution:
                ...
                for entry in group:
                    ...
                    for i in range(entry[1]):
                        if split and i % 2 == 0:
                            entries.insert(0, {"type": botType, "spacing": spacing})
                        else:
                            entries.append({"type": botType, "spacing": spacing})
                if len(entries) > 0:
                    allEntries += entries
                    allEntries.append(
                        {"type": None, "spacing": 40 if random.random() < 0.5 else 80}
                    )
            ...
            wave = {"baseAngle": baseAngle, "entries": allEntries}
        else:
        ...
        for info in entries:
            ...
            if point is not None:
                bs.gameTimer(
                    t, bs.WeakCall(self.addBotAtPoint, point, botType, spawnTime)
                )
                t += dt
            else:
                try:
                ...
                if botType is not None:
                    bs.gameTimer(
                        t, bs.WeakCall(self.addBotAtAngle, botAngle, botType, spawnTime)
                    )
                    t += dt
                botAngle += spacing * 0.5
    ...
    def handleMessage(self, m):
        if isinstance(m, bs.PlayerSpazHurtMessage):
            ...
            if player is None:
                bs.printError(
                    "FIXME: getPlayer() should no" " longer ever be returning None"
                )
                return
            ...
            if m.killerPlayer is not None:
                ...
                # tnt achievement
                    ...
                    if m.badGuy.lastAttackedType == ("explosion", "tnt"):
                        ...
                        if self._tntKills >= 3:
                            bs.gameTimer(
                                500,
                                bs.WeakCall(
                                    self._awardAchievement, "Boom Goes the Dynamite"
                                ),
                            )
                    ...
                    if m.badGuy.lastAttackedType == ("explosion", "tnt"):
                        ...
                        if self._tntKills >= 6:
                            bs.gameTimer(
                                500, bs.WeakCall(self._awardAchievement, "TNT Terror")
                            )
