...
class _Entry(object):
    def __init__(self, scoreboard, team, doCover, scale, label, flashLength):
        ...
        self._barScale = bs.newNode(
            "combine",
            owner=self._bar.node,
            attrs={"size": 2, "input0": self._barWidth, "input1": self._barHeight},
        )

        self._barScale.connectAttr("output", self._bar.node, "scale")

        self._barPosition = bs.newNode(
            "combine", owner=self._bar.node, attrs={"size": 2, "input0": 0, "input1": 0}
        )
    ...
    def _setFlashColors(self, f):
        ...
        if f:
            ...
            _safeSetAttr(
                ...
                "color",
                (
                    self._backingColor[0] * s,
                    self._backingColor[1] * s,
            ...
            _safeSetAttr(
                self._bar.node,
                "color",
                (self._barColor[0] * s, self._barColor[1] * s, self._barColor[2] * s),
            )
            if self._doCover:
                _safeSetAttr(
                    self._cover.node,
                    "color",
                    (
                        self._coverColor[0] * s,
                        self._coverColor[1] * s,
    ...
    def setValue(
        self, score, maxScore=None, countdown=False, flash=True, showValue=True
    ):
        ...
        if self._score is None:
            ...
            if score > self._score or (countdown and score < self._score):
                extraFlash = (
                    maxScore is not None and score >= maxScore and not countdown
        ...
        if maxScore is None:
            ...
            if countdown:
                self._barWidth = max(
                    2.0 * self._scale, self._width * (1.0 - (float(score) / maxScore))
                )
            else:
                self._barWidth = max(
                    2.0 * self._scale, self._width * (min(1.0, float(score) / maxScore))
                )
...
class ScoreBoard(object):
    ...
    def setTeamValue(
        self, team, score, maxScore=None, countdown=False, flash=True, showValue=True
    ):
        """
        ...
        # now set the entry..
        self._entries[team.getID()].setValue(
            score=score,
            maxScore=maxScore,
            countdown=countdown,
            flash=flash,
    ...
    def _addTeam(self, team):
        ...
        self._entries[team.getID()] = _Entry(
            self,
            team,
            doCover=self._doCover,
            scale=self._scale,
            label=self._label,
